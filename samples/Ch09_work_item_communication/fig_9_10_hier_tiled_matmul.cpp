...
double run_sycl(const std::vector<T>& vecA,
  ...
  for (int i = 0; i < iterations; ++i) {
    // Because the hierarchical kernel accumulates directly
    // into the result matrix C, we need to clear the result
    ...
    Q.submit([&](handler& h) {
      ...
      h.parallel_for_work_group(
          num_groups, group_size, [=](group<2> group) {
            // Because this array is declared at work-group
            // scope it is in local memory
            T tileA[16];

            for (int kk = 0; kk < K; kk += tileSize) {
              // A barrier may be inserted between scopes
              // here automatically, unless the compiler can
              ...
              // A barrier gets inserted here automatically,
              // so all work items have a consistent view of
              // memory
