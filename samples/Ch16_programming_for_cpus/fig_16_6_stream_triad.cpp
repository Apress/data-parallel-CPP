...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1) {
    ...
    std::cout
        << "Run as ./<progname> <arraysize in elements>\n";
    std::cout << "Defaulting to an array size of "
              << default_array_size << "\n";
  ...
  std::cout << "Running with stream size of " << array_size
            << " elements ("
            << (array_size * sizeof(float)) / (double)1024 /
                   1024
            << "MB)\n";
  ...
  std::cout << "Time in sec (fastest run): "
            << min_time * 1.0E-9 << "\n";

  double triad_bandwidth =
      1.0E-09 * triad_bytes / (min_time * 1.0E-9);
  std::cout << "Bandwidth of fastest run in GB/s: "
            << triad_bandwidth << "\n";
