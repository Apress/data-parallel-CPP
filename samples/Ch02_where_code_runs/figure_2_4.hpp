...
public:
    ...
    // The command group may be a lambda expression or function object.
    // Returns an event reflecting the status of the action
    ...
    // Wait for all previously submitted actions to finish executing.
    // Pass asynchronous exceptions to an async_handler function.
    void wait_and_throw();
};
