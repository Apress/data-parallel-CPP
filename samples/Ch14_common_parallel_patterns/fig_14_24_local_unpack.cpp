  ...
  Q.parallel_for(
       ...
       [=](nd_item<2> it)
           [[intel::reqd_sub_group_size(8)]] {
             ...
             // Treat each row as a queue of i values to
             // compute Initially the head of the queue is
             // at 0
             uint32_t iq = 0;

             // Initially each work-item in the sub-group
             // works on contiguous values
             ...
             // Keep iterating as long as one work-item has
             // work to do
             while (any_of_group(sg, i < Nx)) {
               uint32_t converged =
                   next_iteration(params, i, j, count, cr,
                                  ci, zr, zi, mandelbrot);
               if (any_of_group(sg, converged)) {
                 // Replace pixels that have converged using
                 // an unpack Pixels that haven't converged
                 // are not replaced
                 uint32_t index = exclusive_scan_over_group(
                     sg, converged, plus<>());
                 i = (converged) ? iq + index : i;
                 iq += reduce_over_group(sg, converged,
                                         plus<>());

                 // Reset the iterator variables for the new
                 // i
                 if (converged) {
                   reset(params, i, j, count, cr, ci, zr,
                         zi);
                 }
               }
             }
